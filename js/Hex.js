function Hex(sideLength, sides) {
    ...
    this.shake = function(obj) {
        var anglePerSide = 360 / this.sides;
        var angle = anglePerSide / 2 + obj.lane * anglePerSide;
        angle *= Math.PI / 180;
        ...
        if (position !== undefined) {
            ...
            if (position <= 0) {
                if (block.distFromHex - block.iter * this.dt * settings.scale - (this.sideLength / 2) * Math.sqrt(3) <= 0) {
                    ...
                    block.settled = 0;
                    var speedModifier = 0.9 + (polygonSides - 5) * 0.05;
                    block.iter = (1.5 + (waveone.difficulty/15) * 3) * speedModifier;
                }
            } else {
    ...
    this.rotate = function(steps) {
        ...
        this.blocks.forEach(function(blocks) {
            blocks.forEach(function(block) {
                block.targetAngle = block.targetAngle - steps * anglePerSide;
            });
        });

        this.targetAngle = this.targetAngle - steps * anglePerSide;
        this.lastRotate = Date.now();
    };
