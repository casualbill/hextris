function Hex(sideLength) {
	...
	this.rotate = function(steps) {
		...
		if (settings.energyEnabled) {
			if (settings.energy < settings.energyCost) {
				// 能量不足，显示警告
				settings.showEnergyWarning = true;
				settings.energyWarningTime = Date.now();
				return;
			} else {
				// 消耗能量
				settings.energy -= settings.energyCost;
			}
		}

		// 冷却时间检查 - 无论设备类型，都应用75ms冷却时间
		if(Date.now()-this.lastRotate<75) {
			return;
		}
